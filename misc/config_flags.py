...
import errno
from importlib import import_module  #import imp
from importlib.machinery import SourceFileLoader # needed on line 457
...
def _LoadConfigModule(name: str, path: str):
  ...
  with ignoring_errors.Attempt('Relative path', path):
    # config_module = imp.load_source(name, path) # MODIFIED BY HAAD
    
    """ START MODIFICATION """
    loader = SourceFileLoader(fullname=name, path=path)
