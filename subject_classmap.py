...
class Subject(object):
    ...
    def reconstruction_gas(self):
        ...
        io_utils.export_nii(np.abs(self.image_gas_highreso), "tmp/image_gas_highreso.nii")
    ...
    def segmentation(self):
        ...
        if self.config.segmentation_key == constants.SegmentationKey.CNN_VENT.value:
            ...
            try:
                ...
                print(f"Haad: mask size after squeezing: {np.shape(self.mask)}")
            except ValueError:
    ...
    def biasfield_correction(self):
        ...
        if self.config.bias_key == constants.BiasfieldKey.SKIP.value:
            ...
            print(f"image (gas_highreso) dims before correction: {np.shape(self.image_gas_highreso)}")
            ...
            print(f"image (gas_highreso) dims after correction: {np.shape(self.image_gas_cor)}")
        else:
    ...
    def dissolved_analysis(self):
        ...
        logging.info("Processed image_rbc2gas")
        ...
        logging.info("Processed image_membrane2gas")


        
    def dissolved_binning(self):
        logging.info("Binning dissolved images to colormap bins")
    ...
    def copy_relevant_files(self):
        """Copy relevant output files into dedicated directory."""
        # define relevant files to copy
        output_path = os.path.join(self.config.data_dir, "output")
        relevant_files = (
            f"{output_path}/mask_reg.nii", # Mask resulting from automatic (or manual?) segmentation
            f"{output_path}/gas_highreso.nii", # original ventilation image
            f"{output_path}/gas_rgb.nii", # color-mapped ventilation image
            f"{output_path}/membrane2gas_rgb.nii", # color-mapped gas-normalized membrane image
            f"{output_path}/rbc2gas_rgb.nii", # color-mapp gas-normalized rbc image
            f"{output_path}/{self.config.subject_id}_report.pdf", # Full report
        ...
        # To make sure membrane2gas.nii appears in .../relevant_files 
        shutil.copy("tmp/membrane2gas.nii", os.path.join(self.config.data_dir, f"{self.config.subject_id}_relevant_files/membrane2gas.nii"))

    def zip_relevant_files(self):
        io_utils.zip_files(base_name=os.path.join(self.config.data_dir, f"{self.config.subject_id}_relevant_files"), 
                           dir_name=os.path.join(self.config.data_dir, f"{self.config.subject_id}_relevant_files"))
